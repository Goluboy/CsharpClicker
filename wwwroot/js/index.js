...
function updateScoreFromApi(scoreData) {
    ...
    if (isFirstTime) {
        const alertElement = document.querySelector('.afk-alert');
        alertElement.textContent = `While you were away you received ${currentScore - prevVal} tadpoles.`;
        setTimeout(() => alertElement.setAttribute('hidden', ''), 5000)
        isFirstTime = false;
    }
